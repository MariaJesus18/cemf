    ...
    <div class="container-xxl  ">
        <form id="formAuthentication" class="mb-3" action="{{route('typeReleases.update',$typeRelease->id)}}" method="POST">
            ...
            <div class="row justify-content-center align-items-center g-2">
                <div class="col">
                    <div class="mb-3">
                        <label for="" class="form-label">{{$typeRelease->title}}</label>
                        <input type="text"
                        class="form-control" name="title" id="" value="{{$typeRelease->title}}" required aria-describedby="helpId" placeholder="Nome da Escola">
                    </div>
